/**
 * Dambrete 8×8 — Krievijas šaškas vai angļu noteikumi (`variant`).
...
/** @typedef {'russian'|'english'} DambreteVariant */

export const DAMBRETE_VARIANT_RUSSIAN = "russian";
export const DAMBRETE_VARIANT_ENGLISH = "english";
...
function isRussianVariant(variant) {
  return String(variant || DAMBRETE_VARIANT_RUSSIAN).toLowerCase() !== DAMBRETE_VARIANT_ENGLISH;
}

function normalizeDambreteVariant(v) {
  return isRussianVariant(v) ? DAMBRETE_VARIANT_RUSSIAN : DAMBRETE_VARIANT_ENGLISH;
}
...
function findManMoves(board, r, c, piece) {
  const moves = [];
  for (const [dr, dc] of manForwardDirs(piece)) {
    const nr = r + dr;
    const nc = c + dc;
...
/** Ķeršana ar vienu lēcienu (2 lauči), virzieni no `dirs` */
function findShortJumps(board, r, c, piece, isWhiteTurn, jumped, dirs, variant) {
  const jumps = [];
  ...
  for (const [dr, dc] of dirs) {
    ...
    if (getPieceColor(mid) === enemy && land === 0 && !jumped.has(midKey)) {
      ...
      const more = findJumps(newBoard, lr, lc, newPiece, isWhiteTurn, newJumped, variant);
      const cap = [[mr, mc]];
      if (more.length) {
...
/** Krievija: bārdiņa ķer 4 virzienos */
function findManJumpsRussian(board, r, c, piece, isWhiteTurn, jumped, variant) {
  return findShortJumps(board, r, c, piece, isWhiteTurn, jumped, DIAG_DIRS, variant);
}

/** Angļu: bārdiņa ķer tikai uz priekšu */
function findManJumpsEnglish(board, r, c, piece, isWhiteTurn, jumped, variant) {
  return findShortJumps(board, r, c, piece, isWhiteTurn, jumped, manForwardDirs(piece), variant);
}

/** Angļu dāma: kā bārdiņa, bet 4 virzienos, viens lēciens */
function findKingJumpsEnglish(board, r, c, piece, isWhiteTurn, jumped, variant) {
  return findShortJumps(board, r, c, piece, isWhiteTurn, jumped, DIAG_DIRS, variant);
}

/** Krievija: dāma — lidojošā ķeršana */
function findKingJumpsRussian(board, r, c, piece, isWhiteTurn, jumped, variant) {
  const jumps = [];
  const enemy = piece > 0 ? BLACK : WHITE;
...
function findJumps(board, r, c, piece, isWhiteTurn, jumped, variant) {
  const v = normalizeDambreteVariant(variant);
  if (isKing(piece)) {
    return v === DAMBRETE_VARIANT_RUSSIAN
      ? findKingJumpsRussian(board, r, c, piece, isWhiteTurn, jumped, v)
      : findKingJumpsEnglish(board, r, c, piece, isWhiteTurn, jumped, v);
  }
  return v === DAMBRETE_VARIANT_RUSSIAN
    ? findManJumpsRussian(board, r, c, piece, isWhiteTurn, jumped, v)
    : findManJumpsEnglish(board, r, c, piece, isWhiteTurn, jumped, v);
}

/** Krievija: dāma slīd */
function findKingMovesRussian(board, r, c, _piece) {
  const moves = [];
  for (const [dr, dc] of DIAG_DIRS) {
    let nr = r + dr;
    let nc = c + dc;
...
/** Angļu: dāma vienu lauciņu */
function findKingMovesEnglish(board, r, c, _piece) {
  const moves = [];
  for (const [dr, dc] of DIAG_DIRS) {
    const nr = r + dr;
    const nc = c + dc;
...
function getAllMoves(board, isWhiteTurn, variant) {
  const v = normalizeDambreteVariant(variant);
  const jumps = [];
  const moves = [];
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      const j = findJumps(board, r, c, piece, isWhiteTurn, new Set(), v);
      if (j.length) jumps.push(...j.map((seq) => ({ jumps: seq })));
      else if (jumps.length === 0) {
        const m = isKing(piece)
          ? v === DAMBRETE_VARIANT_RUSSIAN
            ? findKingMovesRussian(board, r, c, piece)
            : findKingMovesEnglish(board, r, c, piece)
...
function applyMove(board, move, variant) {
  const v = normalizeDambreteVariant(variant);
  const newBoard = board.map((row) => row.slice());
  ...
  if (isKing(piece)) {
    if (v === DAMBRETE_VARIANT_RUSSIAN) {
      ...
      if (Math.abs(tr - fr) !== 1) return null;
    }
  }

  newBoard[fr][fc] = 0;
  const newPiece = promoteIfOnBackRow(piece, tr);
...
function checkGameOver(board, isWhiteTurn, variant) {
  const { jumps, moves } = getAllMoves(board, isWhiteTurn, variant);
