...
function finishHand(state) {
  ...
  /* Beigu UI: acis no noraktajām (tikai «lielais») un no divām vidus kārtīm (zole / mazā zole). */
  if (summary && typeof summary === "object") {
    if (
      contract === "big" &&
      Array.isArray(state.buried) &&
      state.buried.length === 2
    ) {
      let be = 0;
      for (const c of state.buried) be += zoleCardEyes(c);
      summary.buriedEyesTotal = be;
    }
    const kE = Number(state.kittyEyesToOpponents) || 0;
    if (
      (contract === "zole" || contract === "maza_zole") &&
      kE > 0
    ) {
