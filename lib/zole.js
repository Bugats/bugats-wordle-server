...
function emptyStateBase(players) {
  return {
    ...
    /** 1 = pirmā likšanas kārta; 2 = otrā; pēc divām pilnām pasēšanām → Galdiņš (automātiski) */
    bidRound: 1,
...
export function zoleProcessBid(state, playerIdx, bid) {
  ...
  if (b === "pass") {
    ...
    if (state.bidTurn === 0 && state.contractorIdx == null) {
      if (bidRound === 1) {
        ...
        /* Otrā kārta visi pasējuši → Galdiņš (nav «Galds») */
        state.phase = "play";
  ...
  if (b === "galdins") {
    return {
      ok: false,
      error:
        "Galdiņu nepieteic atsevišķi — ja 1. un 2. kārtā visi pasē, tas sākas pats.",
    };
  }
...
function zoleBotTrickSim(trick, playerIdx, card) {
  const row = trick.map((t) => ({ playerIdx: t.playerIdx, card: t.card }));
  row.push({ playerIdx, card });
  return row;
}
...
function zoleBotPickWeakestByEyes(cards) {
  if (!cards.length) return null;
  return cards.reduce((a, b) =>
    zoleCardEyes(a) < zoleCardEyes(b) ? a : b
  );
}
...
function zoleBotPickThird(
  ...
  for (const c of legal) {
    const sim = zoleBotTrickSim(trick, botIdx, c);
    const w = zoleTrickWinner(sim);
    let te = 0;
    for (const t of sim) te += zoleCardEyes(t.card);
    const win = w === botIdx;
    const str = zoleCardTrickStrength(c);
    let score;

    if (isGaldins) {
      score = win ? te * 10 + str : -te * 2 + str * 0.1;
    } else {
      const isContractor = contractorIdx === botIdx;
      if (isContractor) {
        if (win) score = -500 - te + str * 0.05;
        else score = 200 + te + str;
      } else {
        if (win) score = -400 - te * 0.5 + str * 0.05;
        else score = 150 + te * 0.3 + str;
      }
    }

    if (difficulty === "easy" && Math.random() < 0.12) score += Math.random() * 80;
    if (difficulty === "hard" && !isGaldins && contractorIdx !== botIdx && win && trick.some((t) => t.playerIdx === contractorIdx)) {
      score -= 25;
    }
...
/** Otrais stiķī — bez trešās kārtas zināšanas: lēts uzbrukums vai lēta atmesties. */
function zoleBotPickSecond(
  ...
  const losing = [];
  for (const c of legal) {
    const sim = zoleBotTrickSim(state.trick, botIdx, c);
    const w = zoleTrickWinner(sim);
    if (w === botIdx) winning.push(c);
    else losing.push(c);
  }

  if (isGaldins) {
    if (winning.length && difficulty !== "easy") {
      const hi = winning.filter((c) => zoleCardEyes(c) + zoleCardEyes(leadCard) >= 8);
      if (hi.length) return zoleBotPickWeakest(hi);
    }
    if (losing.length) return zoleBotPickWeakestByEyes(losing);
    return zoleBotPickWeakest(winning.length ? winning : legal);
  }

  const isContractor = contractorIdx === botIdx;
  const leaderIsContractor = lead.playerIdx === contractorIdx;

  if (isContractor) {
    if (!leaderIsContractor && winning.length) {
      return zoleBotPickWeakest(winning);
    }
    if (losing.length) return zoleBotPickWeakestByEyes(losing);
    return zoleBotPickWeakest(winning);
  }

  if (leaderIsContractor) {
    if (losing.length) return zoleBotPickWeakestByEyes(losing);
    return zoleBotPickWeakest(winning.length ? winning : legal);
  }
