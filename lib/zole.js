/**
 * Latvijas zole (3 spēlētāji, 26 kārtis) — pēc zolei.lv loģikas.
 * Masti: ♥ kreicis, ♦ ercs, ♣ kāravs, ♠ pīķis.
 * Kāravi: dūzis–septītnieks; pārējie masti: dūzis–devītnieks.
...
/** 0 ♥ kreicis, 1 ♦ ercs, 2 ♣ kāravs, 3 ♠ pīķis */
export const ZOLE_SUIT_CLUBS = 2;
...
/** Dāmu/kalpu stipruma secība mastos: kreicis, pīķis, ercs, kāravs → suit index */
const TRUMP_FACE_ORDER = [0, 3, 1, 2];
...
/** Vai kārta ir trumpis (dāma, kalps, vai parastā kārava) */
export function zoleIsTrump(card) {
  if (!card) return false;
  if (card.r === RANK_Q || card.r === RANK_J) return true;
  if (card.s === ZOLE_SUIT_CLUBS) return true;
  return false;
}
...
export function zoleCardTrickStrength(card) {
  ...
  if (card.s === ZOLE_SUIT_CLUBS) {
    const clubRank = [RANK_7, RANK_8, RANK_9, RANK_K, RANK_10, RANK_A].indexOf(
      card.r
    );
    return 2000 + (clubRank >= 0 ? clubRank : 0);
  }
...
export function createZoleDeck() {
  ...
  const clubRanks = [RANK_A, RANK_10, RANK_K, RANK_Q, RANK_J, RANK_9, RANK_8, RANK_7];
  for (let s = 0; s < 4; s++) {
    const ranks = s === ZOLE_SUIT_CLUBS ? clubRanks : plainRanks;
...
function trumpSuitFromKitty(kitty) {
  for (const c of kitty) {
    if (c.s === ZOLE_SUIT_CLUBS && c.r !== RANK_Q && c.r !== RANK_J) return c.s;
  }
  for (const c of kitty) {
    if (c.r !== RANK_Q && c.r !== RANK_J) return c.s;
  }
  return ZOLE_SUIT_CLUBS;
}

/**
 * Jāatbild ar to pašu mastu (D un J ir attiecīgā masta kārtis).
 */
export function zoleLegalPlays(hand, trick) {
  ...
  const has = hand.some((c) => c.s === leadSuit);
  if (!has) return hand.slice();
...
export function zolePlayCard(state, playerIdx, card) {
  ...
  if (!legal.some((c) => cardKey(c) === cardKey(card))) {
    return { ok: false, error: "Jāievēro masts." };
  }
...
export function zolePublicSnapshot(state, forPlayerIdx) {
  ...
  return {
    ...
    trumpNote:
      "Latviešu trumpji: visas dāmas un kalpi, kā arī pārējās ♣ (kāravas) kārtis.",
    turn: state.turn,
